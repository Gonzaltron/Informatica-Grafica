...
void Mesh::CreateMesh(GLfloat* vertices, GLuint* indices, GLuint numVertices, GLsizei numIndices)
{
    ...
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(GLfloat), (void*)( 6 * sizeof(GLfloat)));
...
void Mesh::RecalculateNormals(GLfloat* vertices, GLuint* indices, GLuint numVertices, GLsizei numIndices, unsigned int numDatosV, unsigned int offsetNormals)
{
    for (int i = 0; i < numIndices; i += 3)
    {
        ...
        glm::vec3 v(vertices[index1] - vertices[index0], vertices[index1 + 1] - vertices[index0 + 1], vertices[index2] - vertices[index2]);
        glm::vec3 u(vertices[index1] - vertices[index0], vertices[index1 + 1] - vertices[index0 + 1], vertices[index2] - vertices[index2]);
